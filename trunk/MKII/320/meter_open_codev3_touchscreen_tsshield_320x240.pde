...
//TODO: 
//-replace oiltemp stuff with LC1 stuff
//-write part for LC-1 for a specific calibration
//-double check that the accel stuff is correct
...
void loop() {
  ...
  while (mouseX != tempX || mouseY != tempY){ //while there is no change in the mouse press
    ...
    four_bar(LC1, boost, temp1, temp2);
...
//TODO:
//-change angle calcs to make sense with LC1 readings
void LC1_gauge(float LC1, float o_LC1){
  float endy, endx;
  float tangle;
  ...
  if (gaugedrawswitch == 0){
    background(0,0,0);
    image(loadImage("LC1back.bmp"),0,0);
    bar_labels();
    gaugedrawswitch = 1;
  }
  ...
  else if ((LC1 > 120) && (LC1 < 180)){ //not ok
    tangle = 1.5 * (LC1 - 120); //1.5 is 90degrees/60degF or the scaling factor
    endx = 87 - (30.0 * lsin(tangle)); 
    endy = 94 + (30.0 * lcos(tangle));
  }
  else if (LC1 == 180){ //necessary because trig does weird things at the 90deg's
    endx = 87 - 30;
    endy = 94;
  }
  else if ((LC1 > 180) && (LC1 < 240)){ //ok
    tangle = 1.5 * (LC1 - 180);
  ...
  else if ((LC1 > 240) && (LC1 < 300)){ //not ok
    tangle = 90 - 1.5 * (LC1 - 240); //inverse...because we are actually looking for the angle down from 90 deg up
    endx = 87 + (30 * lcos(tangle));
    endy = 94 - (30 * lsin(tangle));
  }
  else if (LC1 > 300){//ok
...
//-fix filler calcs for LC-1 bar...should work though; using maxLC1?
void four_bar(float LC_1,float boost, float T1, float T2){
